import { EventSystem, CMD, TARGET, createMessage } from "./eventsystem"
...
class Room {
	...
	protected users: Array<UserSession>;
...
class Server {
	...
	protected eventSystem: EventSystem;	//イベントシステム(イベントをリレーする)
	...
	constructor(port: number) {
		...
		this.eventSystem = new EventSystem((data: any)=>{ this.broadcast(data); });
			...
			ws.on('message', (message: string) => {
				...
				try
				{
					...
					if(this.sessions[sessionId]) {
						this.eventSystem.execMessage(data);
					}else{
	...
	removeSession(uuid: string) {
		...
		this.eventSystem.removeSession(uuid);
